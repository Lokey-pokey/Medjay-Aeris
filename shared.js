...
const GatewayRegistry = (function(){
  ...
  return {
    ...
    update(id, fields){
      ...
      // Polling started earlier holds a closure over the OLD gw object, so a
      // changed baseUrl/apiKey would otherwise keep hitting the stale
      // endpoint until a full page reload. Restart it with the fresh object.
      stopPolling(id);
      ensurePolling(gw);
    },
    getApiKeyPlain(id){
...
// Bar color for the 3 device-alarmable metrics comes from the device's own
// flags bit for that metric (authoritative). Battery has no device
// "warning" tier, only a hard low-battery bit, so we show that directly too.
// Local thresholds (amber-only, client-side) are NOT used here to avoid a
// bar disagreeing with the badge — see LocalThresholds doc note above.
function barClsFor(metric, node){
  const f = decodeFlags(node.flags);
  if(metric==='temp')     return f.temp ? 'alarm' : 'ok';
  if(metric==='humidity') return f.humidity ? 'alarm' : 'ok';
  if(metric==='smoke')    return f.smoke ? 'alarm' : 'ok';
  if(metric==='battery')  return f.batteryLow ? 'warn' : 'ok';
  return 'ok';
}
