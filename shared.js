...
const STORAGE_KEY = "medjayAerisState_v5";
...
function genNode(rand, idx, gwIdx){
  ...
  } else {                        // normal node — every max here must stay
                                   // strictly below its amber threshold, or
                                   // "normal" nodes leak into amber anyway
    tempMax = 30; smokeMax = 28; aqiMax = 75; humidityMax = 66; batteryMin = 35;
  }
...
// Nudges a node's real metric values so it lands in the given severity when
// passed back through computeAlarmState — never assigns alarmState directly.
// Used only to GUARANTEE demo coverage of every bucket (see
// ensureSeverityCoverage), so the badge/filter/sort logic is exercised
// against every case even when random rolls alone wouldn't produce one.
function forceNodeInto(n, target){
  const t = n.thresholds;
  if(target === "red"){
    n.temp = t.temp.red + 2;
  } else if(target === "amber"){
    n.temp = Math.min(t.temp.red - 1, t.temp.amber + 1);
  } else { // "green" — pull every metric comfortably clear of its amber line
    n.temp = Math.max(18, t.temp.amber - 4);
    n.humidity = Math.max(28, t.humidity.amber - 4);
    n.smoke = Math.max(2, t.smoke.amber - 4);
    n.aqi = Math.max(15, t.aqi.amber - 4);
    n.battery = t.battery.amber + 10; // battery is bad-when-low, so "clear" means above amber
  }
  n.alarmState = computeAlarmState(
    {temp:n.temp, humidity:n.humidity, smoke:n.smoke, aqi:n.aqi, battery:n.battery},
    t
  );
  n.ack = n.alarmState === "red" ? false : true;
}

// Random rolls give realistic variety, but nothing in probability alone
// guarantees every severity bucket actually appears — as seen when this
// exact seed happened to always offline the one gateway that would
// otherwise have been the sole "Normal" example. Rather than hand-tune a
// probability constant to paper over one seed's coincidence (fragile: any
// unrelated change to how many rand() calls precede this would silently
// break it again), guarantee coverage explicitly and deterministically.
function ensureSeverityCoverage(gateways){
  const sevOf = gw => gwSeverity(gw);
  const has = s => gateways.some(gw => sevOf(gw) === s);

  if(!has("offline")){
    // Must not carry any red node, or gwSeverity correctly (alarm still
    // outranks offline) reports it as "alarm" instead — silently defeating
    // this guarantee. Prefer a candidate with no red node; if none exists,
    // neutralize whatever red nodes the chosen one has.
    let gw = gateways.find(g => g.online && !g.nodes.some(n => n.alarmState === "red"));
    if(!gw) gw = gateways[0];
    gw.online = false; gw.wifi = false; gw.lora = false;
    gw.nodes.forEach(n => {
      n.online = false;
      if(n.alarmState === "red") forceNodeInto(n, "amber");
    });
  }
  if(!has("ok")){
    const gw = gateways.find(g => g.online && sevOf(g) !== "offline") || gateways[1];
    gw.online = true; gw.wifi = true; gw.lora = true;
    gw.nodes.forEach(n => { n.online = true; forceNodeInto(n, "green"); });
  }
  if(!has("alarm")){
    const gw = gateways.find(g => sevOf(g) !== "offline") || gateways[2];
    forceNodeInto(gw.nodes[0], "red");
  }
  if(!has("warn")){
    const gw = gateways.find(g => sevOf(g) !== "offline" && sevOf(g) !== "alarm") || gateways[3];
    forceNodeInto(gw.nodes[gw.nodes.length-1], "amber");
  }
}
...
function generateInitialState(){
  ...
  const gateways = GATEWAY_NAMES.map((name, gwIdx) => {
    ...
    // them. Realistic ambient rates: mostly independent per-radio rolls,
    // occasionally a correlated site-level outage takes both down together.
    // Bucket *coverage* is no longer this roll's job — see
    // ensureSeverityCoverage below — so these numbers reflect plausible
    // real-world reliability, not demo-display tuning.
    const outageRoll = rand();
    let wifi, lora, gwOnline;
    if(outageRoll < 0.04){
      wifi = false; lora = false; gwOnline = false;
    } else {
      wifi = rand() > 0.08;
      lora = rand() > 0.05;
      gwOnline = wifi || lora;
  ...
  ensureSeverityCoverage(gateways);
...
const MedjayData = (function(){
  ...
  return {
    ...
    setThresholds(nodeId, thresholds){
      ...
      const nowAlarming = node.alarmState === "red";
      if(nowAlarming && !wasAlarming) node.ack = false;
      if(!nowAlarming) node.ack = true;
      save();
      addLog({gateway:res.gateway.id, sensor:nodeId, event:"Threshold Changed", user:"admin"});
      // Editing thresholds is the only way a node's red state can change post-load
      // (there's no live ingestion loop yet), so this transition must be logged
      // explicitly — "Threshold Changed" alone doesn't tell an operator an alarm
      // was actually triggered or cleared as a result.
      if(nowAlarming && !wasAlarming) addLog({gateway:res.gateway.id, sensor:nodeId, event:"Alarm Triggered", user:"admin"});
      if(wasAlarming && !nowAlarming) addLog({gateway:res.gateway.id, sensor:nodeId, event:"Alarm Cleared", user:"admin"});
    },
    toggleBuzzer(nodeId){
      const res = this.getNode(nodeId); if(!res) return;
...
function renderSummaryStripHTML(){
  ...
  const amberWarnings = allNodes.filter(n=>n.alarmState==="amber").length;
  ...
  return `
    ...
    <div class="stat ${amberWarnings>0?'is-warn':''}">
      <div class="label">Active Warnings</div>
      <div class="value-row"><span class="value ${amberWarnings>0?'warn-val':''}">${amberWarnings}</span><span class="sub">across ${gateways.filter(g=>g.nodes.some(n=>n.alarmState==='amber')).length} gateway(s)</span></div>
    </div>
